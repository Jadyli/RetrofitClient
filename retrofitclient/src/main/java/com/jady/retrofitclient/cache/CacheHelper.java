package com.jady.retrofitclient.cache;
...
public class CacheHelper {
    ...
    private void init() {
        ...
        Log.i(TAG, "[Cache Path] " + cacheFile.getPath());
        try {
            cache = DiskLruCache.open(cacheFile, version, valueCout, cacheSize);
        } catch (IOException e) {
            Log.e(TAG, "[DiskLruCache init] " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public File getCacheFile(int type) {
        File dir = new File(mContext.getCacheDir(), "http");
        if (!dir.exists()) {
            dir.mkdirs();
        }
        return new File(dir, "CACHE_" + type);
    }
